...
public class DriverClassUtil {
     ...
     * @throws ClassNotFoundException  if can't find the Class
     */
    public static List<Class<?>> getClasses(String driver) throws ClassNotFoundException {
        List<Class<?>> classList = new ArrayList<Class<?>>();
    	if (StringUtils.isBlank(driver)) {
            return null;
        } else if (StringUtils.contains(driver, ".")) {
        	String[] classNames = driver.split(",");
        	...
        	for(String className : classNames)
        	{
        		classList.add(Class.forName(driverClassBiMap.get(className)));
        	}
            //String fullName = driverClassBiMap.get(driver);
            //return Class.forName(fullName);
        	return classList;
