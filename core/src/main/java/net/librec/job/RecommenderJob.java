...
public class RecommenderJob {
    ...
    private Map<String, List<Double>> cvEvalResults; // list is for getting average results
    
    // organised log entry
    private JobOutput thisJobOutput;
    
    private JobDetails aJobDetails;
    private final String COMMA = ",";
    private StopWatch stopWatch;
    public RecommenderJob(Configuration conf) throws ClassNotFoundException, IOException {
        this.conf = conf;
...
//        }
        ...
        Recommender recommender = (Recommender) ReflectionUtil.newInstance((Class<Recommender>) getRecommenderClass(), conf); // this is where we get the recommendations	        
    ...
    private void generateDataModel() throws ClassNotFoundException, IOException, LibrecException {
    	...
    	dataModel.setDataConvertor(this.dataConvertor);
        dataModel.buildDataModel();
    }

    public void setDataConvertor(DataConvertor c)
    {
    	this.dataConvertor = c;
    }
                    ...
                    for (Map.Entry<MeasureValue, Double> entry : evalValueMap.entrySet()) {
                        ...
                        if (entry != null && entry.getKey() != null) {
                            if (entry.getKey().getTopN() != null && entry.getKey().getTopN() > 0) {
                                ...
                                thisJobOutput.getEvaluationsMap().put(entry.getKey().getMeasure() + " top " + entry.getKey().getTopN(), entry.getValue()); // my
                                ...
                                thisJobOutput.getEvaluationsMap().put(entry.getKey().getMeasure().toString(), entry.getValue()); // my
    ...
    public void saveResult(List<RecommendedItem> recommendedList) throws LibrecException, IOException, ClassNotFoundException {
        if (recommendedList != null && recommendedList.size() > 0) {
            ...
            String outputPath = System.getProperty("user.dir") + File.separator + "result" + File.separator + conf.get("data.input.path") + "-" + algoSimpleName + "-output-" + algoSimpleName;
...
//                System.out.println("IT worked.");
    ...
    private void printCVAverageResult() {
        ...
        for (Map.Entry<String, List<Double>> entry : cvEvalResults.entrySet()) {
            ...
            thisJobOutput.getEvaluationsMap().put(evalName, avgEvalResult);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public Class<? extends Recommender> getRecommenderClass(String className) throws ClassNotFoundException, IOException {
        return (Class<? extends Recommender>) DriverClassUtil.getClass(className);
    }
...
//     */
//    @SuppressWarnings("unchecked")
//    public List<Class<? extends Recommender>> getRecommenderClasses() throws ClassNotFoundException, IOException {
//        return (List<Class<? extends Recommender>>) DriverClassUtil.getClasses(conf.get("rec.recommender.class"));
//    }
    ...
    public void appendResultsHistory()
    {
    	
    }

	public String getJobOutputString() {
		return thisJobOutput.makeString();
	}
    
	public JobOutput getJobOutput()
	{
		return thisJobOutput.getJobOutput();
	}
	
	private void startJob()
	{
		String recommendClassName;
		try
		{
			// get the recommender class simple name and then format it to have spaces between capital letters, adapted from https://stackoverflow.com/a/4886141
			recommendClassName = getRecommenderClass(conf.get("rec.recommender.class")).getSimpleName().replaceAll("(\\p{Ll})(\\p{Lu})","$1 $2");
			 if(!recommendClassName.contains(" Recommender"))
	        {
	        	recommendClassName = recommendClassName.replaceAll("Recommender", " Recommender");
	        }
			 
			String jobID = JobUtil.generateNewJobId();
	        setJobId(jobID); //
	        LOG.info("Job ID: " + jobID); //
	        thisJobOutput = new JobOutput(jobID, ((String) conf.get("data.input.path")), recommendClassName); //
	        
	        String dateStarted = DateUtil.getDateFormat(DateUtil.PATTERN_yyyy_MMM_dd_HH_MM_ss_z).format(new Date());
	        stopWatch = new StopWatch();
	        stopWatch.start();
	        LOG.info("Job started : " + dateStarted); //
	        thisJobOutput.setDateStarted(dateStarted); //
		}
		catch (ClassNotFoundException e)
		{
	...
	{
		String dateFinished = DateUtil.getDateFormat(DateUtil.PATTERN_yyyy_MMM_dd_HH_MM_ss_z).format(new Date());
        LOG.info("Job finished: " + dateFinished); //
        thisJobOutput.setDateFinished(dateFinished); //
        stopWatch.stop();
        thisJobOutput.setTimeTaken(stopWatch.toString());
	}
//	public 

//	public void cancel() throws LibrecException, ClassNotFoundException, IOException
//	{
//		String message = "Cancelled job for dataset " + conf.get("data.input.path") + " and algorithm " + DriverClassUtil.getDriverName(getRecommenderClass()) + ".";
//		throw new LibrecException(message);
//	}
